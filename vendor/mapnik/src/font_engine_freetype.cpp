...
fontnik::face_ptr freetype_engine::create_face(std::string const& family_name)
{
    ...
    if (itr != name2file_.end())
    {
        ...
        if (mem_font_itr != memory_fonts_.end()) // memory font
        {
            ...
            if (!error) return std::make_shared<fontnik::Face>(face);
        }
        else
        {
    ...
    return fontnik::face_ptr();
}


template <typename T>
fontnik::face_ptr face_manager<T>::get_face(std::string const& name)
{
    ...
    {
      fontnik::face_ptr face = engine_.create_face(name);
        if (face)
        {
...
face_set_ptr face_manager<T>::get_face_set(std::string const& name)
{
    ...
    if (fontnik::face_ptr face = get_face(name))
    {
        face_set->add(face);
    }
    else
...
face_set_ptr face_manager<T>::get_face_set(font_set const& fset)
{
    ...
    {
        fontnik::face_ptr face = get_face(name);
        if (face)
        {
            face_set->add(face);
        }
